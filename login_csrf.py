...
def favicon():
    return send_file('static/Sql.ico')


@app.route('/')
def root():
    t = sys_token()
    return render_template('login_with_csrf.html', csrf=t.generate_csrf_token())
...
class LoginV1(Resource):
    ...
    def processRequest(self):
        ...
        print(data)
        t = sys_token()
        if data['csrf'] is None:
            response['code'] = 0
            response['msg'] = 'CSRF Protect!'
            return json.dumps(response)
        else:
            if t.check_csrf_token(data['csrf']) != 0:
                response['code'] = 0
                response['msg'] = 'CSRF Attack Detected'
                return json.dumps(response)
